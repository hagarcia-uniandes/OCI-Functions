...
def handler(ctx, data: io.BytesIO = None):
    ...
    try:
        ...
        with cx_Oracle.connect(user=DB_USER, password=DB_PASSWORD, dsn=dsn_tns) as con:
            ...
            cur = con.cursor()
            EMONCONHOG	= "select nvl(round(sum(amounttotal),0),0) Monto\
                from (select cu.custnum, nvl(sum(round(tmb.accessfee, 1)), 1)  amounttotal\
